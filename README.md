...
- ✅ **Token 验证**：动态 32 位随机令牌，页面加载时实时校验有效性，防止旧会话绕过登录
- ✅ **会话超时**：可配置闲置超时自动登出，监听用户活动自动重置计时
- ✅ **密码加密**：SHA256 哈希存储，修改密码强制验证原密码，默认密码检测与强制修改提醒
- ✅ **角色隔离**：前端路由守卫 + 后端 Token 校验 + API 401 强制退出，三层防护
- ✅ **子管理员限制**：仅超级管理员（admin）可设置用户角色，子管理员仅可查看不可修改
- ✅ **逻辑删除**：合同数据标记删除保留审计记录
- ✅ **操作日志**：全操作持久化到 MongoDB，显示名称历史快照不受后续角色变更影响
- ✅ **超级管理员保护**：admin 账号不可被删除、禁用、修改角色、被他人修改密码
- ✅ **CORS 跨域配置**：可限制允许的源域名
...
- **[新增] 用户信息编辑功能**
  - 用户管理操作列新增「编辑」按钮，管理员可修改用户的邮箱、手机号、部门
  - 弹窗布局与注册页一致，**用户名和真实姓名灰化不可修改**（注册时已固化）
  - 新增 `POST /api/admin/update-user` 后端端点，操作写入审计日志
  - 子管理员（被提升的非 admin 用户）**禁止编辑超级管理员 admin 的信息** → 403

- **[新增] Token 可见性安全加固（防诱骗越权）**
  - 用户管理列表中的「当前Token」列**仅超级管理员（username=admin）可见**
  - 普通用户被提升为管理员角色后，依然**看不到任何用户的 Token**
  - 后端 `list_users` 对非超级管理员直接**不返回 `currentToken` 字段**，前端 `v-if="isSuperAdmin"` 双重防护
  - 杜绝子管理员窃取用户令牌、伪装身份绕过登录获取超级管理员权限的攻击路径

- **[优化] 用户列表精简**
  - 保留「注册时间」列，移除冗余的「创建时间」列（与注册时间字段相同）
  - 修复 `list_users` 中 `verify_admin_token()` 返回值未被捕获的 bug

- **[新增] 合同类型动态管理**
  - 系统设置 → 参数配置新增「合同类型管理」区，管理员可增删改合同类型
  - 合同管理页筛选下拉和表单下拉改为动态渲染，与客户类别、签署公司同模式
  - 后端新增 `contract_types` 配置项及 4 个 CRUD 端点
...
- **[新增] 普通用户/子管理员主页修改密码**
  - 主页导航栏新增「修改密码」按钮（非访客、非超级管理员可见）
  - 修改密码需验证原密码，调用 `/api/change-password` 端点
  - 前端 SHA256 哈希后提交，不传输明文

- **[增强] 管理员密码修改加固**
  - 系统设置中修改管理员密码**必须验证原密码**，原密码错误 → 401
  - 仅更新当前登录管理员自己的密码（不再批量修改所有管理员）
  - 子管理员（被提升的非 admin 用户）禁止使用此功能 → 403，引导前往主页修改

- **[新增] 子管理员权限限制（防套娃）**
  - 仅超级管理员（username=admin）可设置用户角色
  - 子管理员访问用户管理页：可查看用户列表，但**不显示「权限设置」按钮**
  - 后端 `set_user_role` 校验调用者必须为 `admin` 账号，否则返回 403

- **[新增] 角色变更强制重新登录**
  - 管理员修改用户角色后，自动清除目标用户的 `current_token`
  - 目标用户下次请求时 token 失效 → 前端校验不通过 → 跳转登录页 → 登录后获得最新角色权限

- **[修复] 审计日志显示名称历史失真**
  - **根因**：日志写入时已解析显示名称，但读取时 `resolve_display_name()` 会再次解析，导致用户被提升为管理员后旧日志也被改为「管理员XXX」抬头
  - **修复**：`write_log` 同时存储 `user`（原始用户名）和 `displayName`（写入时的历史快照）
  - `get_logs` 优先使用 `displayName` 字段，仅对旧日志做兼容解析
  - 后续角色变更不再影响历史日志的显示

- **[新增] Token 有效性校验（防旧会话绕过登录）**
  - 新增 `/api/verify-token` 端点，前端页面加载时校验 token 是否仍然有效
  - 路由守卫增强：非访客必须同时有 `userRole` 和 `token` 才放行
  - API 返回 401 时自动 `forceLogout()` 跳转登录页
  - 解决终端 A 修改权限后，终端 B 凭旧 localStorage 绕过登录的问题

- **[新增] 会话闲置超时自动登出**
  - 根据系统配置 `session_timeout_minutes` 动态启用
  - 监听 mousemove / keydown / click / scroll 重置活动计时器
  - 每 30 秒检查一次，超时自动强制退出到登录页

- **影响文件**：`auth_service.py`、`settings_service.py`、`database.py`、`auth.py`、`router/index.js`、`contractManager.js`、`systemSettings.js`、`ContractManager.vue`、`SystemSettings.vue`
