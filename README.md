...
跨域处理：CORSMiddleware
数据存储：MongoDB（192.168.1.111:32768）
...
```
数据库系统：MongoDB
服务地址：mongodb://admin:Hr85550780@192.168.1.111:32768/?authSource=admin
数据库名：HRcontract
...
- **Node.js** 16+ 和 npm 8+
- **Python** 3.8+ 和 pip
- **MongoDB** 4.0+ 正常运行（192.168.1.111:32768）
...
3. 后端接收文件 → 保存到 NAS（`\\192.168.1.111\contracts\{timestamp}_{filename}`）
...
### 文件存储位置
```
NAS 路径：\\192.168.1.111\contracts\
